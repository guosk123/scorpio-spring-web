...
import { cancelQueryTask } from '../../appliance/FlowRecords/service';
...
import type { EModelAlias, IFieldProperty } from '../../analysis/components/fieldsManager';
import {
...
import type { IForceGraphProps } from '@/components/ForceGraph';
import { EForceGraphIndex } from '@/components/ForceGraph';
...
import type { INetworkGroupMap } from '../../Configuration/Network/typings';
...
const getInitFilterCondition = ({ networkParamId }: { networkParamId: string }) => {
  const res: IFilter[] = [];
...
const IPGraph: React.FC<IIPGraphProps> = ({
  ...
  const selectedTimeInfo = useMemo(() => {
    if(pcapDetail?.filterStartTime&&pcapDetail?.filterEndTime){
      return timeFormatter(pcapDetail?.filterStartTime, pcapDetail?.filterEndTime);
    }
    return timeFormatter(globalSelectedTime?.startTime,globalSelectedTime?.endTime);
  }, [globalSelectedTime?.endTime, globalSelectedTime?.startTime, pcapDetail]);
  ...
  const [formData, setFormData] = useState<IFormData>(() => {
    ...
    return syncFilterToFormData(getInitFilterCondition({ networkParamId }));
  });
  ...
  useEffect(() => {
    ...
    dispatch({
      type: 'networkModel/qqueryAllNetworkGroups',
    });
  }, []);
  ...
  const getQeryParams = useCallback(
    (drilldownIpAddress?: string) => {
      ...
      let extraDsl = '';
      if (networkId) {
        if (!allNetworkGroupMap[networkId]) {
      ...
      const params: IIpConversationGraphParams = {
          ...
          if (networkId) {
            if (allNetworkGroupMap[networkId]) {
              return {
                ...
                networkGroupId: allNetworkGroupMap[networkId]?.id,
              };
            }
            return {
              sourceType: ESourceType.NETWORK,
        ...
        startTime: pcapFileId ? selectedTimeInfo.startTime : selectedTimeInfo.startTime,
        endTime: pcapFileId ? selectedTimeInfo.endTime : selectedTimeInfo.endTime,
        interval: pcapFileId ? selectedTimeInfo.interval : selectedTimeInfo.interval,
        serviceId: '',
        ...
        minEstablishedSessions,
        count,
        dsl: `${extraDsl} | gentimes timestamp start="${pcapFileId ? selectedTimeInfo.startTime : selectedTimeInfo.startTime
          }" end="${pcapFileId ? selectedTimeInfo.endTime : selectedTimeInfo.endTime}"`,
        // 只要有 IP 查询，就下钻到会话详单
  ...
  const getJumpUrl = useCallback(
    ({
      ...
      if (target === EActionMenuKey.FLOW_RECORD) {
        if (pcapFileId) {
          return getLinkUrl(
            ...
            ).valueOf()}&to=${new Date(globalSelectedTime.originEndTime).valueOf()}&timeType=${ETimeType.CUSTOM
      ...
      if (target === EActionMenuKey.PACKET) {
        if (networkId) {
          return getLinkUrl(
            `/analysis/trace/packet?&filter=${encodeURIComponent(
              JSON.stringify([
                ...packetFilter,
                {
                  field: 'network_id',
...
export default connect(
  ({
    ...
    networkModel: { allNetworks, allNetworkGroupMap },
    settings,
  }: ConnectState) => ({
    globalSelectedTime,
    currentPcpInfo,
