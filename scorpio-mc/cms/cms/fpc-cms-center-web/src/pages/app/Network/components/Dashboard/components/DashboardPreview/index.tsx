...
const DashboardPreview = ({
  ...
  const { serviceId } = useMemo(() => {
    ...
    return { serviceId: urlIds.serviceId };
  }, [urlIds.serviceId]);
  ...
  // 非实时统计
  if (true || realTimeStatisticsFlag === ERealTimeStatisticsFlag.CLOSED) {
      ...
      <Spin spinning={loading}>
        <Row gutter={10}>
          {[
            ...
            .filter(
              (item) => !(item.title === '带宽峰值' && networkType === ENetowrkType.NETWORK_GROUP),
            )
            .map(({ title, value }) => (
