...
import { queryAlertMessageList } from '../service';
import Detail from './Detail';
import styles from './index.less';

const { API_BASE_URL, API_VERSION_PRODUCT_V1 } = appConfig;
          ...
          return (
            <Space>
              <Button
                ...
                size="small"
                onClick={() => {
                  history.push(getLinkUrl(`/flow-trace/flow-record?flowId=${record.flowId}`));
                }}
              >
                会话详单
              </Button>
                      ...
                      DrilldownMenu: (
                        <Menu.ItemGroup>
                          <Menu.Item
                            key="flow-record"
                            onClick={() => {
                              const url = `/flow-trace/flow-record?${stringify({
                                sid: record.sid,
                ...
                <Space>
                  ...
                  <ExportFile
                    ...
                    queryFn={async (params: any) => {
                      window.open(
                        `${API_BASE_URL}${API_VERSION_PRODUCT_V1}/suricata/alert-messages/as-export?${stringify(
                          { dsl, ...params },
                        )}`,
                      );
                    }}
