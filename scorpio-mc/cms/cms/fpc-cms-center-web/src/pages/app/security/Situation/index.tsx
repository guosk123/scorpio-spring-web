...
import { getGeolocationPosition, getLocationInfo } from './utils';
...
const Security = ({ geolocationModel }: ISecurityProps) => {
  const tableActionRef = useRef<ActionType>();

  // 攻击来源地区统计图数据
  const [srcBarData, setSrcBarData] = useSafeState<IPieChartData[]>([]);
  // 攻击目的地区统计图数据
  const [dstBarData, setDstBarData] = useSafeState<IPieChartData[]>([]);
  // 攻击类型分布饼图数据
  const [typePieData, setTypePieData] = useSafeState<IPieChartData[]>([]);
  // 攻击来源地区统计图数据
  const [mapGunData, setMapGunData] = useSafeState<any[]>([]);
  ...
  const [{ startTime, endTime }, setTimeInfo] = useState<{ startTime?: string; endTime?: string }>(
    {},
  );
  ...
  // 获取统计图表数据
  const queryStatData = useCallback(() => {
  ...
  useEffect(() => {
    ...
    // 2. 拉取3个统计
    queryStatData();
  }, [endTime, queryStatData, startTime]);
  ...
  return (
    <FullScreenCard
      title={'安全态势'}
      extra={<TimeAutoRefreshSelectBox onChange={handleTimeChange} />}
    >
            ...
            <div style={{ height: isFullscreen ? 'calc(100vh - 80px)' : 'calc(100vh - 160px)' }}>
              <Row gutter={10} style={{ height: '100%' }}>
                <Col span={7} style={{ height: '100%', zIndex: 10, opacity: 0.6 }}>
                  ...
                  >
                    {typePieData.length === 0 ? <ChartMessage /> : <PieChart data={typePieData} />}
                  </Card>
                  <Card
                    size="small"
                    ...
                    <AutoHeightContainer onHeightChange={(h) => setTableHeight(h - 40 - 40)}>
                      <ProTable<ISuricataAlertMessage>
                        ...
                        request={async (params = {}) => {
                          ...
                          const absPos = getGeolocationPosition(result.content, geolocationModel);
                          setMapGunData(absPos || []);
...
export default connect(({ geolocationModel }: ConnectState) => {
