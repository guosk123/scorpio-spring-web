...
import type { IFilterCondition, IFilterGroup } from '@/components/FieldFilter/typings';
import {
  EFieldOperandType,
  EFieldType,
...
import { expendNetworkGroupConditions } from '@/components/FieldFilter/utils';
import type { IGlobalTime } from '@/components/GlobalTimeSelector';
import type { ConnectState } from '@/models/connect';
import { ESortDirection, ESourceType } from '@/pages/app/analysis/typings';
import { getColumnParams } from '@/pages/app/appliance/FlowRecords/Record';
...
import {
  ...
  exportFlowRecords,
  pingQueryTask,
} from '@/pages/app/appliance/FlowRecords/service';
import type {
  IFlowRecordData,
  IQueryRecordParams,
} from '@/pages/app/appliance/FlowRecords/typings';
import { IOCContext, searchField, tabContent } from '@/pages/app/FlowTrace/IOC';
import { abortAjax, ipV4Regex, isCidr } from '@/utils/utils';
import { StopOutlined } from '@ant-design/icons';
import { useInterval } from 'ahooks';
import { Button, message, Select, Space, Spin, Tooltip } from 'antd';
import type { TablePaginationConfig } from 'antd/es/table/interface';
import { useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react';
import type { Dispatch, NetworkModelState } from 'umi';
...
const DeferedFlowRecord = () => {
  ...
  const { allNetworkGroupMap } = useSelector<ConnectState, NetworkModelState>(
  ...
  // 流日志列声明
  const baseCols = useFlowRecordColumns({
    handlerFilterCondition: setFilterCondition,
    pageProps: { pageSize: pageParams.pageSize, currentPage: pageParams.page },
  ...
  }, [baseCols, sortDirection, sortProperty]);
  ...
  const dsl = useMemo(() => {
    ...
    if (networkId.length > 0) {
      ...
      spl += filterCondition2Spl(
        expendNetworkGroupConditions([netFilter], allNetworkGroupMap),
        filterField,
      );
    }
      ...
      extraTool={
        <div className={styles.header}>
          ...
          <Space direction="horizontal">
            ...
            />
            <ExportFile
              loading={dataLoading || totalLoading || false}
              totalNum={total}
              queryFn={(params: any) => {
