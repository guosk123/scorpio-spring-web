 ...
 * @returns
 */
export function xtoa(hex: number, pad: number) {
...
export function chtoa(ch: number) {
  return ch > 0x1f && ch < 0x7f ? String.fromCharCode(ch) : '.';
}

export function chEscape(ch: string) {
  switch (ch) {
    case '&':
      return '&amp;';
    case '<':
...
const Hexdump: React.FC<IHexdumpProps> = ({ decodeData, highlights = [], loading }) => {
  ...
  const renderHexdump = useCallback(
    (hex: {
      ...
      for (let i = 0; i < pkt.length; i += fullLimit) {
        const strOff = `<span class='${styles.hexdumpOffset}'>${xtoa(i, 4)}</span>`;
        let strHex = '';
        let strAscii = '';
        ...
        for (let j = 0; j < limit; j += 1) {
          ...
          strAscii += chEscape(ascii2Char(code));
  ...
  const { html, tabNames } = useMemo(() => {
    if (!decodeData.bytes) {
      ...
      return renderHexdump({ activeTab, base, decodeTree: decodeData, highlights });
    }
          ...
          <div className={styles.tabWrap}>
            ...
            >
              {tabNames.map((tab, index) => (
                <Radio.Button value={index}>{tab}</Radio.Button>
              ))}
            </Radio.Group>
          </div>
