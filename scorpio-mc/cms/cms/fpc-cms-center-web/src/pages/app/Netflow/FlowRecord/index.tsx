...
const Index = ({ location, globalSelectedTime, sessionDetails, dispatch }: IFlowRecordParams) => {
  // 动态路由参数
  ...
  const updatePageData = async () => {
    const total = await dispatch({
      type: 'netflowModel/querySessionTotalElement',
      payload: {
        queryId,
  ...
  const tableColumns: IFlowRecordColumnProps<IFlowRecord>[] = [
    {
      ...
      width: 170,
      render: (ariseTime) => {
        return moment(ariseTime).add(8, 'h').format('YYYY-MM-DD HH:mm:ss');
      },
    },
    {
      title: '开始时间',
      ...
      ellipsis: true,
      sorter: true,
      render: (ariseTime) => moment(ariseTime).add(8, 'h').format('YYYY-MM-DD HH:mm:ss'),
    },
    {
      title: '结束时间',
      dataIndex: 'end_time',
  ...
  const updateTableData = useCallback(() => {
    ...
    dispatch({
      type: 'netflowModel/querySessionDetail',
      payload: newParams,
    });
...
export default connect(
  ({ netflowModel: { sessionDetails }, appModel: { globalSelectedTime } }: ConnectState) => ({
    globalSelectedTime,
    sessionDetails,
  }),
)(Index as any);
