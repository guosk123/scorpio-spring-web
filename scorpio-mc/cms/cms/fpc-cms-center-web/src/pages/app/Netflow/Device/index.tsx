...
const Device: React.FC<IDeviceParams> = ({ globalSelectedTime, realTimeStatisticsFlag }) => {
  ...
  /** 表单相关方法 */
  // 处理编辑按钮点击
  const handleEdit = (record: INetflowDevice | INetflowDeviceNetif) => {
  ...
  const tableColumns: ProColumns<INetflowDevice | INetflowDeviceNetif>[] = [
    ...
    },
    {
      ...
      sorter: true,
      render: (_, record) => {
        if ((record as INetflowDeviceNetif).ingestBandwidth === 0) {
          return 0;
        }
        return (
          <>
    ...
    },
    {
      ...
      sorter: true,
      render: (_, record) => {
        if ((record as INetflowDeviceNetif).transmitBandwidth === 0) {
          return 0;
        }
        return (
          <>
  ...
  /** 处理设备点击 */
  const handleDevice = (record: INetflowDevice | INetflowDeviceNetif) => {
    const uriPrefix = '/netflow/device';
