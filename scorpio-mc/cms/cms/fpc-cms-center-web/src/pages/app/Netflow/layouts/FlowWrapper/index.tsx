...
const FlowWrapper: React.FC<IFlowWrapper> = ({ dispatch, location: { pathname }, children }) => {
  ...
  // 更新源信息，页面跳转
  const updateSourceUrl = (deviceName: string, netifNo?: string) => {
    let dstUrl = pathname;
  ...
  /** 获取当前源的详细信息 */
  const querySourceDetails = async () => {
    const result = await dispatch({
      type: 'netflowModel/Search',
      payload: { keywords: urlParams.deviceName },
    });
    const sources: ISearchParam[] = result;
    const sourceInfo = (() => {
      if (sources) {
