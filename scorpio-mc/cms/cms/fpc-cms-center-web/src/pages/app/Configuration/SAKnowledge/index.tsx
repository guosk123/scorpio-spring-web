...
class ApplicationList extends React.PureComponent<ApplicationPolicyProps, ApplicationListState> {
    ...
    if (customApplicationList.length >= MAX_CUSTOM_APPLICATION_LIMIT) {
      return (
        <Tooltip title={`最多支持新建${MAX_CUSTOM_APPLICATION_LIMIT}个自定义应用`}>
          <Button icon={<PlusOutlined />} type="primary" disabled={true}>
            新建自定义应用
          </Button>
        </Tooltip>
      );
  ...
  renderDeleteBtn = (id: string, type: ECustomSAApiType, children?: any[]) => {
  ...
  render() {
        ...
        <Result
          ...
          extra={
            <Button
              ...
              onClick={() =>
                history.push(getLinkUrl('/configuration/knowledge/sa-knowledge/upload'))
              }
            >
              上传SA规则库
            </Button>
    ...
    const applicationColumns: ColumnProps<ApplicationItem>[] = [
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          return (
            <Fragment>
              <Button type="link" size="small" onClick={() => this.handleShowDrawer(id!)}>
                详情
              </Button>
    ...
    return (
      <Skeleton active loading={queryAllApplicationsLoading} style={{ height: '100%' }}>
        <ConnectCmsState
        ...
        <Row gutter={10}>
          <Col span={5}>
            ...
            <Card
              ...
              extra={
                <Link to={getLinkUrl('/configuration/objects/sa-knowledge/subcategory/create')}>
                  新建
                </Link>
              }
              className={styles.categoryCard}
            ...
            <section className="searchForm small">
              <Row style={{ marginBottom: 10 }} gutter={10}>
                ...
                <Col xl={24}>
                  ...
                  >
                    <Form.Item name="applicationName" initialValue={searchApplicationName}>
