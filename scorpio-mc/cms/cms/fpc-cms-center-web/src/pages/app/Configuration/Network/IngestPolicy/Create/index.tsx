...
import { IIngestPolicy } from '../../../IngestPolicy/typings';
...
}> = ({ dispatch, allNetworkSensor, allIngestPolicy = [] }) => {
  // 表单ref
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        const success = await dispatch({
          type: 'networkModel/createNetworkingestPolicy',
          payload: values,
        });
        if (success) {
          Modal.success({
  ...
  return (
    ...
    >
      ...
      <Form.Item wrapperCol={{ offset: 5, span: 16 }}>
        ...
        <Button
          onClick={() => {
            history.push('/configuration/netflow/ingest/network-ingest-policy');
          }}
        >
          取消
        </Button>
