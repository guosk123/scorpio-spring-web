  ...
  if (category === EAlertCategory.TREND) {
    ...
    const resultText = `基线值为<b>${trendBaselineText}</b>，实际值<b>${enumValue2Label(
      ...
      alertFireContext?.trendPercent
        ? '，趋势百分比为' + '<b>' + `${formatterResult(alertFireContext.trendPercent)}` + ' </b>'
        : ''
    }`;
...
const AlertMessageContent: React.FC<IAlertMessageProps> = (props) => {
  const { dispatch, queryDetailLoading } = props;
  const [alertRuleDetail, setAlertRuleDetail] = useState<any>({});
  const [detail, setDetail] = useState<IAlertMessage>({} as any);
  const [state, alertDispatch] = useContext<any>(AlertContext);
  const [description, setDescription] = useState<string>('');
  // const { alertId }: { alertId: string } = useParams();
  const [alertId] = useState(state?.shareInfo?.alertId);
  useEffect(() => {
    clearShareInfo(alertDispatch);
    if (dispatch) {
      dispatch({
  ...
  }, [dispatch, alertId, alertDispatch]);
