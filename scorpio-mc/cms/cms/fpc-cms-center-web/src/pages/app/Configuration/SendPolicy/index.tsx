...
import { queryNetworkGroups, queryNetworkSensors } from '../Network/service';
...
export default function SendPolicy() {
  ...
  const fetchData = async () => {
    ...
    if (success) {
      const { result: allNetworks } = await queryNetworkSensors();
      const { result: allNetworkGroups } = await queryNetworkGroups();
      const policyList = [];
      for (const policy of result) {
        const list = policy?.quote?.filter((f: any) => f) || [];
        const res = [];
        for (const i of list) {
          const network = (allNetworks || []).find((n: any) => n?.networkInSensorId === i);
          const group = (allNetworkGroups || []).find((n: any) => n?.id === i);
          if (network) {
  ...
  const columns: ColumnsType<ISendPolicy> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Switch
            disabled={record?.quote?.length !== 0}
            checkedChildren="启用"
      ...
      render: (_: any, record: any) => (
        <>
          <Button
            ...
            size="small"
            onClick={() => {
              history.push(`/configuration/transmit/send-policy/${record?.id}/update`);
            }}
          >
            编辑
          </Button>
          ...
          >
            <Button
              type="link"
              size="small"
              style={{ color: record?.quote?.length !== 0 ? 'gray' : 'red' }}
              disabled={record?.quote?.length !== 0}
            >
              删除
            </Button>
      ...
      <div style={{ marginBottom: 10, textAlign: 'right' }}>
        <Space>
          ...
          <Button
            ...
            icon={<PlusOutlined />}
            onClick={() => {
              history.push('/configuration/transmit/send-policy/create');
            }}
          >
            新建
          </Button>
