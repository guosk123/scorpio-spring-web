...
export default function SendPolicyForm({ id }: { id?: string }) {
  ...
  >(undefined);
  const [isQuote, setQuote] = useState<boolean>(false);
      ...
      onOk: async () => {
        if (id) {
          ...
          if (success) {
            ...
            history.push('/configuration/transmit/send-policy/list');
            return;
          }
          message.error('编辑失败!');
        } else {
    ...
    if (id) {
      (async () => {
        ...
        if (success) {
          ...
          setQuote(result?.quote?.length > 0);
      ...
      <div style={{ marginTop: '30px' }}>
        <Form {...layout} onFinish={onFinish} form={form}>
          ...
          <Form.Item name={'state'} label={'启用'}>
            <Checkbox disabled={isQuote} checked={state} onChange={(e) => setState(e.target.checked)} />
          </Form.Item>
