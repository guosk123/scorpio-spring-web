...
import type { INetworkSensor } from '../../../Network/typings';
...
const LogicalSubnet: React.FC<LogicalSubnetProps> = (props) => {
  ...
  const handleCreate = (values: any) => {
    values.networkInSensorIds = values.networkId.join(',');
    delete values.networkId;
    createConfirmModal({
  ...
  const checkGreKey = async (rule: any, value: string) => {
    ...
    if (!/^[0-9,]+$/.test(value)) {
      return Promise.reject('只能输入大小写字母、数字和半角逗号,');
    }
  ...
  return (
    <Card bordered={false}>
      <Form
        ...
        initialValues={{
          ...detail,
          networkId: detail.networkInSensorIds?.split(','),
          // 处理各种配置
          insideIpAddress:
      ...
      >
        ...
        >
          <Select placeholder="选择所属网络" loading={queryAllNetworkSensorLoading} mode="multiple">
            {allNetworkSensor.map((network) => (
              <Select.Option key={network.networkInSensorId} value={network.networkInSensorId}>
                {network.name}({network.sensorName})
