...
import { DoubleRightOutlined, LeftSquareOutlined, ReloadOutlined } from '@ant-design/icons';
import { Popover, TreeProps } from 'antd';
...
const SearchTree: React.FC<ISearchTreeProps> = ({
  ...
  const defExpandKeys = useMemo(() => {
    ...
    setExpandedKeys(expandedKeys.concat(res));
    return res;
  }, [treeKeysMap]);
              ...
              <AutoHeightContainer contentStyle={{ overflowY: 'auto', overflowX: 'scroll' }}>
                <Tree
                  ...
                  titleRender={(data: any) => {
                    if (data?.status === '1') {
                      return (
                        <Popover placement="right" content={data?.statusDetail} trigger="hover">
