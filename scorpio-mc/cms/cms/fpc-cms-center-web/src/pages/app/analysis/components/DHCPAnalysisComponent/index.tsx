...
import { MetaDataContext } from '@/pages/app/appliance/Metadata';
import { jumpToMetadataTab } from '@/pages/app/appliance/Metadata/constant';
...
const renderLink = (
  ...
  let uriPrefix = `/analysis/network/${networkId}`;
  if (pcapFileId) {
...
const DHCPAnalysisComponent: React.FC<IDHCPAnalysisProps> = (props) => {
  ...
  const dsl = useMemo(() => {
    ...
    } AND dhcp_version=${dhcpType}) | gentimes timestamp start="${
      globalSelectedTime.startTime
    }" end="${globalSelectedTime.endTime}"`;
    return nextDsl;
  }, [filterCondition, networkId, filterFields, dhcpType, globalSelectedTime]);
