...
class SystemStateMetricsChart extends PureComponent {
  ...
  handleResult = () => {
    ...
    monitorMetricsHistogram.forEach(
      ({ metricTime, memoryMetric = 0, cpuMetric = 0, systemFsMetric = 0 }) => {
        const time = moment(metricTime).valueOf();
        memoryUageArr.push([time, memoryMetric]);
        cpuUsageArr.push([time, cpuMetric]);
        systemFsUageArr.push([time, systemFsMetric]);
      },
    );
