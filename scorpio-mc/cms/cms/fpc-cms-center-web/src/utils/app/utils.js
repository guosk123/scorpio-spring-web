...
export function validateBpfStr(value) {
  ...
  return ajax(
    `${API_VERSION_PRODUCT_V1}/appliance/bpf-rule-verifications?bpf=${valueToBase64}`,
  ).then((response) => {
    const { success, result } = response;
    if (success) {
      if (result === true) {
...
export const onlyNumber = (item) => {
  if (typeof parseFloat(item) === 'number' && parseFloat(item).toString() !== 'NaN') {
    return item;
  } else {
    return 0;
  }
