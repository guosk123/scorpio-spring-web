...
import { ipV4Regex, ipV6Regex, isCidr, isObject } from '@/utils/utils';
import {
  ClearOutlined,
  CloseSquareOutlined,
  FilterOutlined,
...
export const getFilterContent = (
  ...
  if (dsl) {
    ...
    if (
      (!field.operandType || field.operandType === EFieldOperandType.STRING) &&
    ...
    if (field?.type) {
      r += `${filter.field}<${field.type}> ${operator}`;
    } else {
      r += `${filter.field} ${operator}`;
    }
    if (!isExistsOperator(operator)) {
      r += field?.operandType === EFieldOperandType.NUMBER ? ` ${operand}` : ` "${operand}"`;
    }
  } else {
    // 返回标签字符串
...
export const filterCondition2Spl = (
  ...
  let sql = '';
  filterCondition.forEach((filter, index) => {
    // 如果不是filterGroup
    if (filter.hasOwnProperty('operand')) {
      const r = getFilterContent(filter as IFilter, true, fields, customFormatter);
      if (r) {
        sql += `(${r})`;
      }
    } else {
      const r = getFilterGroupContent(filter as IFilterGroup, true, fields, customFormatter);
      if (r) {
        sql += `(${r})`;
      }
    }
    if (index !== filterCondition.length - 1) {
      sql += ' AND ';
    }
  });
...
export const renderOperatorContent = (fieldId: string, fields: IField[], simple?: boolean) => {
  const { type, operandType } = fields.find((field) => field.dataIndex === fieldId) || {};
  let operatorList = [];
  ...
  if (type === EFieldType.ARRAY && operandType === EFieldOperandType.NUMBER) {
    ...
    operatorList = enumObj2List({ ...eqOperator, ...likeOperator });
  } else {
      ...
      rules={[
        ...rules,
        {
          validator: async (rule, value) => {
            ...
            // console.log('custom validator us ', validator);
            // console.log('custom validtor res:', res);
...
const FilterCondition: React.FC<IFilterConditionProps> = ({
  ...
  const handleCheck = async () => {
    try {
      ...
      const filter = {
        ...
        disabled: (() => {
          if (values?.field?.length === 1 && values?.field[0]) {
            return fields.find((f) => f.dataIndex === values?.field[0])?.unEditable === true;
          }
          return false;
        })(),
      ...
      // 清空原有字段
      form.resetFields();
      form.setFieldsValue({ operand: undefined });
    } catch (errorInfo) {
  ...
  // 找到能够选择过滤的字段
  const enableFields = fields.filter((item) => {
    if (item.disabled) {
      return false;
    }
    if (item.single) {
      return (
        filterCondition.findIndex((f) => {
          if (f.hasOwnProperty('operand')) {
            return (f as IFilter).field === item.dataIndex;
          }
          return false;
        }) === -1
      );
  ...
  form.setFieldsValue({
    field: enableFields.length > 0 ? [enableFields[0].dataIndex] : undefined,
    operator: EFilterOperatorTypes.EQ,
    operand: undefined,
  });
    ...
    <div className={styles.filterWrap}>
              ...
              <FormItem noStyle name="field">
                <Cascader
                  ...
                  options={fields.map((field) => {
                    if (field.type !== EFieldType.Map) {
                      const singledCondition = filterCondition.find((c) => {
                        if ((c as IFilterGroup)?.group) {
                          return false;
                        } else {
                          if ((c as IFilter)?.field === field.dataIndex && field?.single) {
                            return true;
                          }
                        }
                        return false;
                      ...
                      return {
                        disabled: !!singledCondition || field.disabled,
                        label: field.title,
      ...
      {modalVisible && (
        <AdvancedFilter
          ...
          fields={fields.filter((item: IField) => {
            return !item.disabled && !item.single;
          })}
