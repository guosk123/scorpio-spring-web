DROP TABLE IF EXISTS fpc.d_fpc_metric_port_data_record;
CREATE TABLE IF NOT EXISTS fpc.d_fpc_metric_port_data_record
(
`timestamp` DateTime64(3, 'UTC') COMMENT '时间',
`network_id` String COMMENT '网络ID',
`service_id` String COMMENT '业务ID',
`port` UInt16 COMMENT '端口',
`ip_protocol` String COMMENT '四层协议类型(tcp/udp)',
...
ENGINE = Distributed(ch_stats_servers, fpc, t_fpc_metric_port_data_record)
COMMENT '端口统计表';