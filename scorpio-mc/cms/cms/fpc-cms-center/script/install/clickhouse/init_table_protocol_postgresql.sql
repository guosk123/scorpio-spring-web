DROP TABLE IF EXISTS fpc.d_fpc_protocol_postgresql_log_record;
CREATE TABLE IF NOT EXISTS fpc.d_fpc_protocol_postgresql_log_record
(
...
`database_name` LowCardinality(String) COMMENT '数据库名',
`cmd` String COMMENT '执行的SQL语句',
`error` LowCardinality(String) COMMENT 'SQL执行错误',
`delaytime` UInt32 COMMENT 'SQL执行时延(ms)'
) 
ENGINE = Distributed(clickhouse_servers, fpc, t_fpc_protocol_postgresql_log_record)
COMMENT '应用层协议详单-PostgreSQL协议详单表';