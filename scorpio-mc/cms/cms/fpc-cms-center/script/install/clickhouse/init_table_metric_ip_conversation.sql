DROP TABLE IF EXISTS fpc.d_fpc_metric_ip_conversation_data_record;
CREATE TABLE IF NOT EXISTS fpc.d_fpc_metric_ip_conversation_data_record
( 
`timestamp` DateTime64(3, 'UTC') COMMENT '记录时间',
`network_id` String COMMENT '网络ID',
`service_id` String COMMENT '业务ID',
`ip_a_address` String COMMENT 'IP地址A',
`ip_b_address` String COMMENT 'IP地址B',
...
ENGINE = Distributed(ch_stats_servers, fpc, t_fpc_metric_ip_conversation_data_record)
COMMENT 'IP通讯对统计表';