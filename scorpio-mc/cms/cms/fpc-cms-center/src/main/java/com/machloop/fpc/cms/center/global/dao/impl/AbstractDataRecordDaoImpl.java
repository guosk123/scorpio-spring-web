...
import com.machloop.fpc.cms.center.global.dao.DataRecordDao;
...
public abstract class AbstractDataRecordDaoImpl implements DataRecordDao {
   ...
   * @return 统计结果
   */
  protected Map<String, Object> metricAggregate(String tableName, MetricQueryVO queryVO,
      List<Map<String, Object>> additionalConditions,
    ...
    if (!StringUtils.equals(queryVO.getColumns(), "*")) {
      ...
      fields.addAll(getCombinationAggsFields("timestamp"));
    ...
    Iterator<
        Entry<String, Tuple2<AggsFunctionEnum, String>>> iterator = aggFields.entrySet().iterator();
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("metricAggregate sql : [{}], param: [{}] ", sql.toString(), params);
    }
  ...
  protected List<Map<String, Object>> metricDataConversion(List<Map<String, Object>> metricResult) {
    ...
    metricResult = metricResult.stream().map(map -> map.entrySet().stream()
  ...
  protected String convertTableName(String sourceType, int interval, String packetFileId) {
    if (StringUtils.equals(sourceType, FpcCmsConstants.SOURCE_TYPE_PACKET_FILE)) {
      String packetFileTableName = String.join("_", getTableName(), packetFileId);
      ...
      List<Map<String, Object>> existResult = getClickHouseJdbcTemplate().getJdbcTemplate()
          .query(String.format("show tables from %s where name = '%s'",
              CenterConstants.FPC_DATABASE, packetFileTableName), new ColumnMapRowMapper());
