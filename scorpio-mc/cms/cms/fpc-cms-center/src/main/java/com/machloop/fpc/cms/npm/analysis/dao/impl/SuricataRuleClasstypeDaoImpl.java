...
public class SuricataRuleClasstypeDaoImpl implements SuricataRuleClasstypeDao {
  ...
  private static final String TABLE_ANALYSIS_SURICATA_RULE_CLASSTYPE = "fpccms_analysis_suricata_rule_classtype";
  ...
  @Override
  public List<String> querySuricataRuleClasstypeIds(Boolean onlyLocal) {
    StringBuilder sql = new StringBuilder();
    sql.append("select id from ").append(TABLE_ANALYSIS_SURICATA_RULE_CLASSTYPE);
    sql.append(" where deleted = :deleted ");
  ...
  public List<SuricataRuleClasstypeDO> querySuricataRuleClasstypes(Date beforeTime) {
    StringBuilder sql = new StringBuilder();
    sql.append("select id, assign_id from ").append(TABLE_ANALYSIS_SURICATA_RULE_CLASSTYPE);
  ...
  public SuricataRuleClasstypeDO saveSuricataRuleClasstype(
    ...
    sql.append(" (id, assign_id, name, deleted, create_time, update_time, operator_id)");
    sql.append(" values(:id, :assignId, :name, :deleted, :createTime, :updateTime, :operatorId) ");

    if (StringUtils.isBlank(suricataRuleClasstypeDO.getId())) {
      suricataRuleClasstypeDO.setId(IdGenerator.generateUUID());
    }
    if (StringUtils.isBlank(suricataRuleClasstypeDO.getAssignId())) {
      suricataRuleClasstypeDO.setAssignId("");
    }
  ...
  public int saveSuricataRuleClasstypes(List<SuricataRuleClasstypeDO> suricataRuleClasstypes) {
    ...
    suricataRuleClasstypes.forEach(suricataRuleClasstype -> {
      ...
      if (StringUtils.isBlank(suricataRuleClasstype.getAssignId())) {
        suricataRuleClasstype.setAssignId("");
      }
