...
import com.machloop.fpc.cms.center.metadata.data.ProtocolDB2LogDO;
...
public class ProtocolDB2LogDaoImpl extends AbstractLogRecordDaoImpl<ProtocolDB2LogDO>
  ...
  private static final String TABLE_NAME = "d_fpc_protocol_db2_log_record";
  ...
  private String addConditions(boolean isFilterIp, Set<String> sortProperties, String columns) {
    String resultString = "";
    ...
    // 必须查询排序字段
    sortProperties = sortProperties.stream()
        .filter(sortProperty -> !StringUtils.contains(columns, sortProperty))
        .collect(Collectors.toSet());
    String selectFields = CsvUtils.convertCollectionToCSV(sortProperties);
