 ...
 * @author chenshimiao
 * <p>
 * create at 2022/12/9 5:09 PM,cms
 * @version 1.0
 */
@Repository
public class DomainWhiteListDaoImpl implements DomainWhiteListDao {

  private static final String TABLE_APPLIANCE_DOMAIN_WHITE = "fpccms_appliance_domain_whitelist";
  ...
  @Override
  public DomainWhiteListDO queryDomainWhiteListByAssignId(String assignId) {
    StringBuilder sql = buildSelectStatement();
  ...
  @Override
  public List<String> queryDomainWhiteListIds(boolean onlyLocal) {
    StringBuilder sql = new StringBuilder();
    sql.append("select id from ").append(TABLE_APPLIANCE_DOMAIN_WHITE);
  ...
  @Override
  public List<String> queryAssignDomainWhiteListIds(Date beforeTime) {
    StringBuilder sql = new StringBuilder();
    sql.append("select id from ").append(TABLE_APPLIANCE_DOMAIN_WHITE);
    ...
    List<String> ids = jdbcTemplate.queryForList(sql.toString(), params, String.class);
    return CollectionUtils.isEmpty(ids) ? Lists.newArrayListWithCapacity(0) : ids;
  }

  @Override
  public List<String> queryDomainWhiteListInCmsId() {
    StringBuilder sql = new StringBuilder();
    sql.append("select assign_id from ").append(TABLE_APPLIANCE_DOMAIN_WHITE);
    sql.append(" where deleted = :deleted and assign_id != '' ");
  ...
  public List<DomainWhiteListDO> saveDomainWhite(List<DomainWhiteListDO> domainWhiteListDOList,
    ...
    sql.append(" (id, assign_id, name, domain, description, ");
    sql.append(" deleted, create_time, update_time, operator_id ) ");
    sql.append(" values (:id, :assignId, :name, :domain, :description, ");
    ...
    for (DomainWhiteListDO domainWhiteListDO : domainWhiteListDOList) {
      ...
      if (StringUtils.isBlank(domainWhiteListDO.getAssignId())) {
        domainWhiteListDO.setAssignId("");
      }
      domainWhiteListDO.setCreateTime(DateUtils.now());
      domainWhiteListDO.setUpdateTime(domainWhiteListDO.getUpdateTime());
      domainWhiteListDO.setDeleted(Constants.BOOL_NO);
      domainWhiteListDO.setOperatorId(id);
  ...
  @Override
  public int deleteDomainWhiteListAll(boolean onlyLocal, String cmsAssignment) {
    StringBuilder sql = new StringBuilder();
    sql.append("update ").append(TABLE_APPLIANCE_DOMAIN_WHITE);
    ...
    params.put("operatorId", cmsAssignment);
