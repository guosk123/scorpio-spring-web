package com.machloop.fpc.cms.baseline.setting.service.impl;
...
import com.machloop.fpc.cms.baseline.calculate.OperationFactory;
import com.machloop.fpc.cms.baseline.mission.Metric;
import com.machloop.fpc.cms.baseline.mission.Mission;
import com.machloop.fpc.cms.baseline.mission.MissionService;
import com.machloop.fpc.cms.baseline.mission.Window;
import com.machloop.fpc.cms.baseline.mission.WindowModelEnum;
 ...
 */
@Service
public class BaselineSettingServiceImpl implements BaselineSettingSyncService {

  private static final Logger LOGGER = LoggerFactory.getLogger(BaselineSettingServiceImpl.class);
  ...
  /**
   * @see com.machloop.fpc.cms.baseline.setting.service.BaselineSettingSyncService#sync()
   */
  @Override
  ...
  private Mission createMission(BaselineSettingDO setting) {
    ...
    if (StringUtils.equals(setting.getCategory(), FpcCmsConstants.BASELINE_CATEGORY_BANDWIDTH)) {
      isRatio = true;
      try {
    ...
    try {
      Metric metric = new Metric(
          isRatio, numeratorMetric, sourceType, StringUtils.joinWith(",", setting.getNetworkId(),
              setting.getNetworkGroupId(), setting.getServiceId()),
          null, null, null, denominatorConstants);
      ...
      mission = new Mission(id, FpcCmsConstants.BASELINE_SETTING_SOURCE_NPM, id,
          setting.getNetworkId(), setting.getNetworkGroupId(), setting.getServiceId(), metric,
          operation, window);
    } catch (IllegalArgumentException e) {
  ...
  private String getMetricField(String category) {
    ...
    switch (category) {
      ...
      case FpcCmsConstants.BASELINE_CATEGORY_PACKET:
        ...
        break;
      case FpcCmsConstants.BASELINE_CATEGORY_RESPONSELATENCY:
