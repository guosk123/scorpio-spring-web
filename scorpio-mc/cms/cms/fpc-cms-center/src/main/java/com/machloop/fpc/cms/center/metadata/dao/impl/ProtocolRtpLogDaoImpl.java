...
public class ProtocolRtpLogDaoImpl extends AbstractLogRecordDaoImpl<ProtocolRtpLogDO>
  ...
  private static final String TABLE_NAME = "d_fpc_protocol_rtp_log_record_analysis";
  ...
  @Override
  public Tuple2<String, List<String>> queryLogRecord(LogRecordQueryVO queryVO, List<String> ids,
      Sort sort, int size) {
    // 数据源
    String tableName = convertTableName(queryVO);
  ...
  protected ProtocolRtpLogDO convertLogMap2LogDO(Map<String, Object> map) {
    ...
    if (map.containsKey("srcIp")) {
      String srcIp = MapUtils.getString(map, "srcIp");
      ...
      protocolRtpLogDO.setSrcIp(srcIp);
    }

    if (map.containsKey("destIp")) {
      String destIp = MapUtils.getString(map, "destIp");
      ...
      protocolRtpLogDO.setDestIp(destIp);
    }
    ...
    String inviteSrcIp = MapUtils.getString(map, "inviteSrcIp");
    if (StringUtils.isNotBlank(inviteSrcIp)
        && StringUtils.startsWith(inviteSrcIp, CenterConstants.IPV4_TO_IPV6_PREFIX)) {
      inviteSrcIp = StringUtils.substringAfter(inviteSrcIp, CenterConstants.IPV4_TO_IPV6_PREFIX);
    }
    protocolRtpLogDO.setInviteSrcIp(inviteSrcIp);
    protocolRtpLogDO.setInviteSrcPort(MapUtils.getIntValue(map, "invite_src_port"));
    String inviteDestIp = MapUtils.getString(map, "inviteDestIp");
    if (StringUtils.isNotBlank(inviteDestIp)
        && StringUtils.startsWith(inviteDestIp, CenterConstants.IPV4_TO_IPV6_PREFIX)) {
      inviteDestIp = StringUtils.substringAfter(inviteDestIp, CenterConstants.IPV4_TO_IPV6_PREFIX);
    }
  ...
  protected String buildSelectStatement(String columns, Set<String> sortPropertys) {
    if (StringUtils.equals(columns, "*")) {
      return " level,policy_name,flow_id,network_id,service_id,application_id,start_time,invite_time,from,IPv6NumToString(src_ip) as srcIp,src_port,to,IPv6NumToString(dest_ip) as destIp,dest_port,ip_protocol,"
          + "IPv6NumToString(invite_src_ip) as inviteSrcIp,invite_src_port,IPv6NumToString(invite_dest_ip) as inviteDestIp,invite_dest_port,invite_ip_protocol,sip_flow_id,"
    ...
    if (StringUtils.isBlank(columns)) {
      ...
      return result.replace(",src_ip", ",IPv6NumToString(src_ip) as srcIp")
          .replace(",dest_ip", ",IPv6NumToString(dest_ip) as destIp")
          .replace("invite_src_ip", "IPv6NumToString(invite_src_ip) as inviteSrcIp")
          .replace("invite_dest_ip", "IPv6NumToString(invite_dest_ip) as inviteDestIp")
