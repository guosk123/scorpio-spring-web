...
public class MetricDnsDataRecordDaoImpl implements MetricDnsDataRecordDao {
  ...
  private static final String TABLE_NAME = "d_fpc_protocol_dns_log_record";
  ...
  protected String convertTableName(LogRecordQueryVO queryVO) {
    ...
    if (StringUtils.equals(queryVO.getSourceType(), FpcCmsConstants.SOURCE_TYPE_PACKET_FILE)) {
      tableName = String.join("_", tableName, queryVO.getPacketFileId());

      // 判断离线数据包文件的分析结果是否存在
      List<Map<String, Object>> existResult = jdbcTemplate.getJdbcTemplate().query(String
