...
const SqlConverter: React.FC<SqlConverterProps> = ({
  ...
  const triggerChange = (changedValue: Record<string, unknown>) => {
    if (onChange) {
      onChange({
        ...value,
        ...changedValue,
  ...
  const handleChange = useCallback(
    // 移除防抖，防抖回导致光标直接跳转到最后
    (q: Record<string, unknown>) => triggerChange(q),
    [],
  );
    ...
    try {
      ...
      if (fields.length > 0) {
        const autoCompleteFieldValues = fields.map(
          ({ value: fieldValue, type }) => fieldValue + (type ? `<${type}>` : ''),
        );
  ...
  useEffect(() => {
    handleChange({ spl, dsl });
  }, [dsl, handleChange, spl]);
