...
import { queryRuntimeEnvironments } from '@/services/app/metric';
...
const AppModel: AppModelType = {
  ...
  effects: {
    *queryRouterPreData(_params, { call }) {
      yield call('networkModel/queryNetworkTree');
    },
  ...
  subscriptions: {
    setup({ dispatch, history: listenHistory }) {
      let prevPathname = listenHistory.location.pathname;
      let preQuery = (listenHistory.location as ILocation).query;
      return listenHistory.listen((location) => {
        const { pathname, search, query } = location as ILocation;
        // 登录界面有参数的情况下清空参数
        ...
        // 从url获取时间
        const { timeType = ETimeType.CUSTOM, from, to, unit, relative } = query;
        ...
        if (initGlobalTime) {
          ...
          // 由于这里的客户端时间已经被设置成了服务器时间
          // 所以这里需要重新初始化一下时间，替换掉客户端时间
          // SecurityLayout/index.tsx 中已经设置过一次了，这里应该没必要再保留了
          // const newDefaultTime = getDefaultTime();
          // dispatch({
          //   type: 'updateState',
          //   payload: {
          //     globalTime: newDefaultTime,
          //     globalSelectedTime: newDefaultTime,
          //   },
          // });
        }
      });
