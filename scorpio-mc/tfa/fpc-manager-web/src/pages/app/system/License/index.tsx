...
const Countdown = ({ license }: { license: ILicenseInfo }) => {
  ...
  return (
    <span className={styles.normal}>
      <CheckCircleOutlined /> {moment.duration(diffDays, 'days').humanize()}
    </span>
  );
};
    ...
    <div className={styles.meta}>
      <div className={styles.left}>
        <Descriptions size="small" column={1}>
          ...
          {packetTimeLimit && (
            <Descriptions.Item label="数据包最大存储时长">{packetTimeLimit}天</Descriptions.Item>
          )}
          {packetCapacityLimit && (
            <Descriptions.Item label="数据包最大存储空间">
              {packetCapacityLimit >= ONE_KILO_1024
                ? `${packetCapacityLimit / ONE_KILO_1024} TiB`
                : `${packetCapacityLimit} GiB`}
            </Descriptions.Item>
          )}
          {recvSpeedLimit && (
            <Descriptions.Item label="收包限速">
              {convertBandwidth(recvSpeedLimit * 8 || 0)}
            </Descriptions.Item>
          )}
        </Descriptions>
      </div>
