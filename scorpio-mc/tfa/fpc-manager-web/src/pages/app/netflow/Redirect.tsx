    ...
    if (originStartTime && originEndTime) {
      querySources({
        ...
        if (content && content.length > 0) {
          ...
          if (deviceName) {
            if (netifNo) {
              history.push(`/analysis/netflow/device/${deviceName}/netif/${netifNo}/dashboard`);
            } else {
              history.push(`/analysis/netflow/device/${deviceName}/dashboard`);
            }
            setHasSource(true);
            return;
          }
  ...
  if (!hasSource) {
    return (
      <Result
        ...
        title="无Netflow源"
        extra={
          <Button type="primary" onClick={() => history.push('/analysis/netflow/device/list')}>
            配置源
          </Button>
        }
      />
