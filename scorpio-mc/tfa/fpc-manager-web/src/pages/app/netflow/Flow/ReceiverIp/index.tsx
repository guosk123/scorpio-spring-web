...
import type { IFlowIngestIp } from '../../typing';
...
import { queryFlowIngestIpHist } from '../../service';
...
interface IFlowParams extends AppModelState {
  tableColumns: ProColumns<IFlowIngestIp>[];
  pageName?: string;
  location: {
    pathname: string;
  };
...
const ReceiverIp = ({ globalSelectedTime, location: { pathname } }: IFlowParams) => {
  // 改维度支持的过滤条件
  const { filterCondition, addConditionToFilter } = useContext(FilterContext);
  // 表格列定义
  const tableColumns: ProColumns<IFlowIngestIp>[] = [
    {
      title: 'IP地址',
      dataIndex: 'ipAddress',
      width: '20%',
    ...
    {
      title: '总字节数',
      dataIndex: 'ingestBytes',
      align: 'center',
      search: false,
      sorter: true,
      width: '20%',
      render: (_, record) => {
        if(record.ingestBytes&&(parseInt(record.ingestBytes,10))!==0){
          return bytesToSize(parseInt(record.ingestBytes, 10))
        }
        return 0;
      },
    },
    {
      title: '总包数',
      dataIndex: 'ingestPackets',
      align: 'center',
      search: false,
      sorter: true,
      width: '20%',
      render: (_, record) => {
        return numeral(parseInt(record.ingestPackets, 10)).format('0,0');
      },
    },
    {
      title: '操作',
  ...
  return (
    ...
    <Template
      pageName="ingest-ip"
      initialRank="ingestBytes"
      filterCondition={filterCondition}
      tableColumns={tableColumns}
      addConditionToFilter={addConditionToFilter}
      queryFunction={queryFlowIngestIpHist}
    />
  );
};
