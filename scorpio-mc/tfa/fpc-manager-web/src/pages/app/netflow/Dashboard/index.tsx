...
import type { IDashbroad, ILineApiData, IUrlParams } from '../typing';
...
const Profiles = ({
  ...
  // 获取Dasbboard信息
  const [dashboardData, setDashBoardData] = useState<IDashbroad>();
  // 加载标志
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const { success, result } = (await queryDashboard({
        ...
        interval,
        netifSpeed,
      })) as IAjaxResponseFactory<IDashbroad>;
      setLoading(false);
      if (success) {
  ...
  // 区分ipv4和ipv6
  const getIpVersion = (lineData?: ILineApiData) => {
    if (lineData === undefined) {
      return '';
    }
    return lineData.ipv4Address ? 'ipv4Address' : 'ipv6Address';
  };
    ...
    <Row gutter={[10, 10]} className={styles.container}>
              ...
              <>
                ...
                <h2>
                  {dashboardData?.totalBandwidth !== undefined
                    ? convertBandwidth(parseInt(dashboardData?.totalBandwidth, 10))
                    : 0}
                </h2>
              </>
            )}
      ...
      <Col span={12}>
        <MultipleSourceTrend
          ...
          onBrush={handleChartBrush}
          data={(() => {
            let lineData = lineChartConverter(
              protocolPortData,
              'protocolPort',
              'totalBandwidth',
              undefined,
...
export default connect(
  ({
    netflowModel: { selectedNetifSpeed },
