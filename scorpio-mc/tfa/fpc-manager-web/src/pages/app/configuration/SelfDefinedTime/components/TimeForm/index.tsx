...
import { createSelfDefinedTime, updateSelfDefinedTimeDetail } from '../../services';
...
const TimeSetting: React.FC<TimeConfigItemFormSharedProps> = ({ timeDetail, pageMode }) => {
  ...
  const [timeArr, setTimeArr] = useState(() => {
    const updateTimeArr = Object.keys(timeDetail || {})
      ...
      .map((item) => ({ name: item }));
    return updateTimeArr.length ? updateTimeArr : [{ name: `time1` }];
  });
  ...
  const submitData = async () => {
    ...
    if (EPageMode.Create == pageMode) {
      const { success } = await createSelfDefinedTime(data);
      if (!success) {
        message.error('创建失败!');
        return;
      }
