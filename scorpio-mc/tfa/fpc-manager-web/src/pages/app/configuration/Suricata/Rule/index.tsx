...
import useQueryLoading from '@/hooks/useQuery';
import type { ConnectState } from '@/models/connect';
import {
  RuleDirectionOptions,
  RuleParseStateOptions,
  RuleProtocolOptions,
...
import { intelligenceRuleDesc } from '../../AbnormalEvent/Rule/components/ImportRule';
...
const SuricataRuleList = () => {
  ...
  const cmsFlagRef = useRef(cmsFlag);
  cmsFlagRef.current = cmsFlag;

  // 重新初始化页面依赖数据，ruleSource,ruleMitre,ruleClasstype
  ...
  const importClasstypeLoading = useSelector<ConnectState, boolean | undefined>(
    (state) => state.loading.effects['suricataModel/importSuricataClasstype'],
  );

  const importThreatIntelligenceLoading = useSelector<ConnectState, any>(
    (state) => state.loading.effects['suricataModel/importThreatIntelligenceRule'],
  );
  ...
  const columns: ColumnProps<ISuricataRule>[] = useMemo(
    () => [
      ...
      {
        title: '解析状态',
        dataIndex: 'parseStateText',
        search: false,
        align: 'center',
        width: 100,
        render: (value, record) => {
          const { parseState } = record;
          if (parseState === '1') {
            return value;
          }
          return <Tooltip title={record.parseLog}>{value}</Tooltip>;
        },
      },
      {
        title: '操作',
        ...
        render: (_, record) => {
          ...
          return (
            <Button
              ...
              size="small"
              // 系统内置不可编辑,但是sid为1的除外
              disabled={cmsFlag || (source === ERuleSource.系统内置 && id !== '1')}
              onClick={() => {
                history.push(
    ...
    [classtypeDict, cmsFlag, mitreDict, sources],
  );
  ...
  const handleCmsConnectFlagChange = (flag: boolean) => {
    setCmsFlag(flag);
  };

  const [enableAllLoading, handleEnableAll] = useQueryLoading(enableAllSuricataRules);

  const [disableAllLoading, handleDisableAll] = useQueryLoading(disableAllSuricataRules);

  const [deleteAllLoading, handleDelAll] = useQueryLoading(async () => {
  ...
  return (
    <div className={styles.ruleList}>
      <ConnectCmsState onConnectFlag={handleCmsConnectFlagChange} />
      <AutoHeightContainer
        autoHeight={true}
          ...
          <>
            ...
            >
              ...
              <Row justify="space-between" gutter={24}>
                {formExpand && (
                  <Col span={6}>
                    <Form.Item label="解析状态" name="parseState">
                      <Select options={RuleParseStateOptions} allowClear />
                    </Form.Item>
                  </Col>
                )}
                <Col span={18} offset={formExpand ? 0 : 6}>
                  <Row justify="end">
                    <Space align="start">
                      <Button
                      ...
                      <Button
                        key="batch"
                        onClick={handleBatch}
                        disabled={cmsFlag || selectedRules.length > 0}
                      >
                        批量操作
                      </Button>
                      ...
                      <Input.Group compact style={{ whiteSpace: 'nowrap' }}>
                        ...
                        >
                          <Button loading={enableAllLoading} disabled={cmsFlag}>
                            全部启用
                          </Button>
                        </Popconfirm>
                        <Popconfirm
                        ...
                        >
                          <Button loading={disableAllLoading} disabled={cmsFlag}>
                            全部停用
                          </Button>
                        </Popconfirm>
                        <Popconfirm
                          ...
                          title="是否确定全部删除"
                          disabled={cmsFlag}
                          onConfirm={handleDelAll}
                        >
                          <Button danger loading={deleteAllLoading} disabled={cmsFlag}>
                            全部删除
                          </Button>
                        </Popconfirm>
                      </Input.Group>
                          ...
                          overlay={
                            <Menu>
                              ...
                              <Menu.Item key="classtype">
                                <Import
                                  loading={importClasstypeLoading}
                                  modalTitle="规则分类导入"
                                  buttonText="规则分类"
                                  disabled={cmsFlag}
                                  buttonProps={{
                              ...
                              <Menu.Item key="threat-intelligence-rule">
                                <Import
                                  loading={importThreatIntelligenceLoading}
                                  modalTitle="威胁情报导入"
                                  buttonText="威胁情报"
                                  buttonProps={{ type: 'text', size: 'small', block: true }}
                                  importFunc="suricataModel/importThreatIntelligenceRule"
                            ...
                            <Menu>
                              ...
                              <Menu.Item key="threat-rule">
                                <Button
                                  ...
                                  onClick={() => {
                                    window.open(
                                      `${API_BASE_URL}${API_VERSION_PRODUCT_V1}/analysis/threat-intelligences/as-export`,
                                    );
                                  }}
                                >
                                  威胁情报
                  ...
                  <Space size={16}>
                    {!cmsFlag && (
                      <>
                        ...
                        {selectedRules.filter((rule) => rule.source === ERuleSource.系统内置)
                          .length < 1 && (
                          <a
                            onClick={() => {
      ...
      >
        <ClasstypeTable actionDisable={cmsFlag} />
      </Modal>
      <Modal
