...
// import {ECustomTimeType} from '../../../../SelfDefinedTime/typings';
// import { querySelfDefinedTime } from '../../../../SelfDefinedTime/services';
...
class AlertRuleForm extends PureComponent<IAlertRuleFormProps, IAlertRuleFormState> {
  ...
  // queryAlertTimes = () => {
  //   querySelfDefinedTime().then((res) => {
  //     const { success, result } = res;
  //     if (success) {
  //       const nameList: any = result.map((item) => ({name: item.name, id: item.id, type: item.type })).filter((i)=>(i.type === ECustomTimeType.PeriodicTime));
  //       // console.log(nameList, 'nameList');
  //       this.setState({ alertTimes: nameList });
  //     }
  //   });
  // };
  ...
  render() {
    ...
    // const { alertTimes } = this.state;
    const { getFieldValue } = form;
    const networkAndSubNet = networkTree.concat(
        ...
        {getFieldValue('category') === EAlertCategory.TREND && (
          <FormItem key="trendDefine" {...formLayout} label="基线定义" required>
            <Card bodyStyle={{ padding: '20px 10px 0' }}>
              ...
              {/* <FormItem label="告警时间" key="customTimeId" {...formLayout}>
                {form.getFieldDecorator('customTimeId', {
                  initialValue: detail.customTimeId || '',
                })(
                  <Select
                    showSearch
                    style={{ width: 200 }}
...
export default connect(
  ({
    ...
    queryAllSourceLoading:
      effects['serviceModel/queryAllServices'] || effects['networkModel/queryNetworkTree'],
  }),
