...
import type { IMoveAppPoliciesParams} from './service';
import { importFilterRules } from './service';
  ...
  const columns: ProColumns<IApplicationPolicy>[] = [
    {
      ...
      render: (text, record, index) => {
        if (cmsConnectFlag) {
          return (pageData?.page || 0) * (pageData?.pageSize || 0) + index + 1;
        }
        return (
          <>
    ...
    },
    {
      ...
      search: false,
      render: (text, record) => {
        const networkIdList = record?.networkId?.split(',').filter((n) => n) || [];
        if (networkIdList.length === 0) {
          return '-';
        }
        return networkIdList.reduce((pre, cur, index) => {
          return (
            pre +
            (allNetworks.find((n) => n.id === cur)?.name || cur) +
            (index !== networkIdList.length - 1 ? ',' : '')
          );
        }, '');
      },
    },
      ...
      render: (text, record) => {
        return (
          <Checkbox
            ...
            disabled={record.id === DEFAULT_POLICY_ID || record.disabled || cmsConnectFlag}
            onChange={(e) => {
              alterPolicyState(
                record?.id,
        ...
        return (
          <>
            <Button
              type="link"
              size="small"
              disabled={record.disabled || cmsConnectFlag}
              onClick={() => {
                // 查找数据并解析,设置初始化值
    ...
    return (
      <div>
        <Button
          type="link"
          size="small"
          disabled={selectedRowKeys.length > 1 || cmsConnectFlag}
          onClick={() => {
        ...
        <Dropdown
          ...
          disabled={cmsConnectFlag}
        >
          <Button disabled={cmsConnectFlag} type="link" size="small">
            移动
          </Button>
        </Dropdown>
        <Popconfirm
      ...
      <DragSortTable<IApplicationPolicy>
        ...
        optionRender={() => [
          <Import
            ...
            tempDownloadUrl={`/appliance/filter-rules/as-template`}
            disabled={cmsConnectFlag}
          />,
          <Button
            type="primary"
...
export default connect(
  ({ SAKnowledgeModel: { allCategoryList }, networkModel: { allNetworks } }: ConnectState) => {
    return { allCategoryList, allNetworks };
  },
)(ApplicationPolicy);
