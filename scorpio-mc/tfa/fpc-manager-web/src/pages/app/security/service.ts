...
import type { IPacketsDownloadPreview } from '../appliance/components/DownLoadPktBtn';
import type {
  IMitreAttack,
  IRuleClasstype,
...
export async function queryAlertPackets(
  params: any,
): Promise<IAjaxResponseFactory<IPacketsDownloadPreview>> {
  return ajax(`${API_VERSION_PRODUCT_V1}/suricata/alert-messages/packets?${stringify(params)}`);
}
