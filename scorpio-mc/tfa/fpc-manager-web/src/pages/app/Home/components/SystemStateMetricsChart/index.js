...
class SystemStateMetricsChart extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      fsSystemTotalByte: 0,
      // 详单总分区
      fsMetadataTotalByte: 0,
      // 详单热分区
      fsMetadataHotTotalByte: 0,
      // 索引分区
      fsIndexTotalByte: 0,
    };
  }
  ...
  componentDidMount() {
    ...
    queryMetrics().then((res) => {
      ...
      if (success) {
        let tempfsSystemTotalByte = 0;
        let tempfsMetadataTotalByte = 0;
        let tempfsMetadataHotTotalByte = 0;
        let tempfsIndexTotalByte = 0;
        result.forEach((item) => {
          if (item.metricName === 'fs_system_total_byte') {
            tempfsSystemTotalByte = Number(item.metricValue);
          }
          if (item.metricName === 'fs_metadata_total_byte') {
            tempfsMetadataTotalByte = Number(item.metricValue);
          }
          if (item.metricName === 'fs_metadata_hot_total_byte') {
            tempfsMetadataHotTotalByte = Number(item.metricValue);
          }
          if (item.metricName === 'fs_index_total_byte') {
            tempfsIndexTotalByte = Number(item.metricValue);
          }
        });
        this.setState({
          fsSystemTotalByte: tempfsSystemTotalByte,
          fsMetadataHotTotalByte: tempfsMetadataHotTotalByte,
          fsMetadataTotalByte: tempfsMetadataTotalByte,
          fsIndexTotalByte: tempfsIndexTotalByte,
        });
      }
    });
  }
  ...
  handleResult = () => {
    ...
    monitorMetricsHistogram.forEach(
      ({
        ...
        systemFsUageArr.push([time, systemFsUsedRatio, systemFsFree]);
        metadataFsUsedArr.push([time, metadataFsUsedRatio, metadataFsFree]);
        metadataHotFsUsedArr.push([time, metadataHotFsUsedRatio, metadataHotFsFree]);
        indexFsUsedArr.push([time, indexFsUsedRatio, indexFsFree]);
      },
    );
      ...
      tooltip: {
        formatter: (params) => {
          ...
          params.forEach((v) => {
            ...
            let restBytes = 0;
            let needAppendRestTotal = false;
            if (v.seriesName === '系统分区使用率') {
              needAppendRestTotal = true;
            }
            if (v.seriesName === '索引分区使用率') {
              needAppendRestTotal = true;
            }
            if (v.seriesName === '详单冷分区使用率') {
              needAppendRestTotal = true;
            }
            if (v.seriesName === '详单热分区使用率') {
              needAppendRestTotal = true;
            }
            restBytes = v.value[2] || 0;

            if (needAppendRestTotal) {
              label += `（剩余容量: ${bytesToSize(restBytes, 3, 1024)}）`;
            }
