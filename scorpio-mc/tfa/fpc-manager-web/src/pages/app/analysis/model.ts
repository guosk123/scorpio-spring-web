...
const Model = modelExtend(pageModel, {
  ...
  effects: {
    ...
    *queryNetworkFlow({ payload }, { call, all, put, select }) {
      const { chartQueryId, ...restParams } = payload as IFlowQueryParams;
      // queryIds ==> chart query id
      const [histogramResponse] = yield all(
        chartQueryId
          ? [
          ...
          : [call(queryNetworkFlowHistogram, restParams)],
      );
      ...
      const newData: { histogramData: any[] } = {
      ...
      return {
        // 返回是否被取消的状态
        abort: !histogramResponse.status,
      };
    },
    *queryNetworkFlowTable({ payload }, { call, all, put, select }) {
      const { tableQueryId, ...restParams } = payload as IFlowQueryParams;
      // queryId ==> table query id
      const [tableResponse] = yield all(
        tableQueryId
          ? [call(queryNetworkFlow, { ...restParams, queryId: tableQueryId })]
          : [call(queryNetworkFlow, restParams)],
      );
      const preData = select((state: ConnectState) => {
        return {
