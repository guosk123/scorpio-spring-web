...
import type { HttpAnalysisResult, IUriParams } from '../typings';
...
const HTTPAnalysis: React.FC<IHTTPAnalysisProps> = ({
  ...
  const dsl = useMemo(() => {
    return `${
      networkId || pcapFileId ? `(network_id=${networkId || pcapFileId})` : ''
    } | gentimes timestamp start="${globalSelectedTime.startTime}" end="${
      globalSelectedTime.endTime
    }" `;
  }, [networkId, pcapFileId, globalSelectedTime.startTime, globalSelectedTime.endTime]);
