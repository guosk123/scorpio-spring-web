...
export default function RTP() {
        ...
        return (
          <>
            ...
            <span
              className="link"
              onClick={() => {
                let url = '/analysis/trace/flow-record';
                ...
                url = `${url}?filter=${encodeURIComponent(
                  JSON.stringify(filterParam),
                )}&from=${startTimestamp}&to=${endTimestamp}&timeType=${ETimeType.CUSTOM}`;
                jumpNewPage(url);
              }}
            ...
            <span
              className="link"
              onClick={() => {
                let url = `/analysis/trace/metadata/record?jumpTabs=${EMetadataTabType.SIP}`;
                const srcIp = record?.inviteSrcIp;
                const destIp = record?.inviteDestIp;
  ...
  const dsl = useMemo(() => {
    ...
    if (network?.id) {
      ...
      filterSpl += `( network_id<Array> = ${network?.id} )`;
    }
    return filterSpl;
  }, [filterCondition, filterField, network?.id]);
  ...
  }, [columns, dsl, endTime, network?.id, sortDirection, startTime, totalElements]);
