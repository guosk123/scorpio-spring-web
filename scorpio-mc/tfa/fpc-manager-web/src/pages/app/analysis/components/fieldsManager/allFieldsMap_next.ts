...
export function getEnumValueFromModelNext(alias: EModelAlias): modelEnumData | null {
  ...
  switch (alias) {
    case EModelAlias.network: {
      ...
      const networkList = (originData as NetworkModelState).allNetworks.map((item) => ({
        text: item.name,
        value: item.id,
      }));
      const networkMap = (originData as NetworkModelState).allNetworkMap;
      ...
      listData = [...networkList, ...allLogicalSubnets];
      mapData = {
        ...networkMap,
        ...allLogicalSubnetMap,
      };
