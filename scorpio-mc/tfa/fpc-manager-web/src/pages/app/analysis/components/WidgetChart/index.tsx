...
function WidgetChart<RecordType extends Record<string, any>>({
  ...
  const urlFragment = useMemo(() => {
    ...
    if (pcapFileId) {
      ...
      res = `performance/service/${serviceId}/${networkId}`;
    } else if (networkId) {
      res = `performance/network/${networkId}`;
    }
    return res;
  ...
  const handleMenuClick = useCallback(
    (info: any) => {
      if (!['time', 'flow-record', 'packet'].includes(info.key)) {
      ...
      if (['flow-record', 'packet'].includes(info.key)) {
        if (pcapFileId) {
          const url = getLinkUrl(
            `/analysis/${urlFragment}/${info.key}?from=${brushTime[0]}&to=${
              brushTime[1]
            }&timeType=${ETimeType.CUSTOM}&filter=${encodeURIComponent(JSON.stringify(filter))}`,
          );
          jumpNewPage(url);
        } else {
          const url = getLinkUrl(
    ...
    [dispatch, urlFragment, brushTime, changeGlobalTime, pcapFileId, filter],
  );
  ...
  }, [disableChangeTime, handleMenuClick]);
