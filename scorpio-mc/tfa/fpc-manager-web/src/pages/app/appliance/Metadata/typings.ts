...
import type { EMetadataCollectLevel } from '../../configuration/MetadataCollectPolicy/typings';
...
export const opTypeMap = new Map([
  ['0', 'other'],
  ['1', 'bind'],
  ['2', 'search'],
  ['3', 'unbind'],
...
export const FileRestoreStateLabel = {
  [EFileRestoreState.full]: '完整',
  [EFileRestoreState.truncation]: '截断',
  [EFileRestoreState.error]: '错误',
  [EFileRestoreState.unknown]: '未知',
};
...
export const resStatusMap = new Map([
  ['0', '-'],
  ['1', 'Success'],
  ['2', 'Failed'],
]);
...
export enum E_STANDARD {
  IS_STANDARD = '1',
  IS_NOT_STANDARD = '0',
}

export const protocolStandardMap = {
  [E_STANDARD.IS_NOT_STANDARD]: '非标协议',
  [E_STANDARD.IS_STANDARD]: '标准协议',
};
...
export interface IMetadataDns extends IMetadataCommonField {
  ...
  domainIntelligence: DOMAIN_CATEGORY;
  /** 子域名数量 */
...
export interface IMetadataSip extends IMetadataCommonField {
  from: string;
  to: string;
...
export interface IMetadataLdap extends IMetadataCommonField {
  /** 请求类型 */
  opType: EOpType;
  /** 回复状态 */
  resStatus: EResStatus;
  /** 请求内容 */
  reqContent: string;
  /** 回复内容 */
  resContent: string;
...
export interface IMetadataDb2 extends IMetadataCommonField {
  codePoint: string;
  data: Record<string, string>;
}

export interface IMetadataSocks4 extends IMetadataCommonField {
  //'请求命令'
  cmd: string;
  //'远端端口',
  requestRemotePort: number;
  //'远端IP',
  ...
  cmdResult: string;
  //'服务器返回的远端IP ',
  responseRemoteIp: string;
  //'服务器返回的远端端口',
  responseRemotePort: number;
}

export interface IMetadataFile extends IMetadataCommonField {
  l7Protocol?: string;
  state: EFileRestoreState;
  md5?: string;
  sha1?: string;
  sha256?: string;
  name: string;
...
export enum LORState {
  success = '1',
  fail = '0',
}

export const LORToText = {
  [LORState.success]: '成功',
  [LORState.fail]: '失败',
};

export const LORToYN = {
  [LORState.success]: '是',
  [LORState.fail]: '否',
};
