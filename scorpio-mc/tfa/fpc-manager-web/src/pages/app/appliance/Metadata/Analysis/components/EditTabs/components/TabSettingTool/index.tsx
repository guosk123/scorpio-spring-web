...
export default function TabSettingTool(props: Props) {
  ...
  // tabs tool使用
  const [tabsToolSetting, setTabsToolSetting] = useState<string[]>(
    JSON.parse(storage.get(tabsKey) || '[]'),
  );
  ...
  const selectObj = useMemo(() => {
    ...
    const safeTabKeysFlag =
      tabsToolSetting.length && !tabsToolSetting.some((item: string) => !item.length);
  ...
  const ColumnSettingMenu = useMemo(() => {
    const onChangeGroup = (e: CheckboxValueType[]) => {
      setTabsSetting(e);
      setTabsToolSetting(e.map((i) => i.toString()));
