  ...
  if (params.dsl?.indexOf('tcp_established_fail_flag=1') !== -1) {
    return ajax(
      `${API_VERSION_PRODUCT_V1}/appliance/flow-logs/establish-fail${
        ...
        `?${stringify({
          ...params,
          dsl: params.dsl?.replace('AND (tcp_established_fail_flag=1)', ''),
        })}`
      }`,
    );
  }
  return ajax(`${API_VERSION_PRODUCT_V1}/appliance/flow-logs?${stringify(params)}`);
}
 ...
 * @param queryId
 */
export async function cancelQueryTask({
  queryId,
}: IHeartbeatParams): Promise<IAjaxResponseFactory<any>> {
