...
import type { IL7Protocol, IL7ProtocolMap } from '../../../Metadata/typings';
...
export const FIELD_FOR_NUMBER = [FIELD_PORT, FIELD_SOURCE_PORT, FIELD_DEST_PORT];
...
const FilterTuple: React.FC<IFilterTupleProps> = ({
  ...
  const checkNumberRange = (arr: number[], min: number, max: number, type?: string) => {
    ...
    if (type === 'port') {
      if (strOne.indexOf('.') > -1 || strTwo.indexOf('.') > -1) {
        throw new Error('请输入正确的端口值');
      }

      if (arr.length == 1 && String(~~strOne).length !== strOne.length) {
        throw new Error('请输入正确的端口值!');
      }
      if (
        arr.length == 2 &&
        (String(~~strTwo).length !== strTwo.length || String(~~strOne).length !== strOne.length)
      ) {
        throw new Error('请输入正确的端口值!!');
      }
    }
  ...
  const checkOperand = (rule: any, operand: string, groupIndex: number) => {
    ...
    const operandArr = operand ? String(operand).split('-') : [];
    ...
    if (portFields.indexOf(field) > -1) {
      checkNumberRange(operandArr as any, PORT_MIN_NUMBER, PORT_MAX_NUMBER, 'port');
      return;
    }
  ...
  const [portValue, setPortValue] = useState<string | undefined>();

  /** 根据字段判断显示操作数的DOM */
  const renderOperand = (field: string) => {
    const sizeStyle = smallSize ? { width: 120 } : { width: 200 };
    if (field === FIELD_APPLICATION_ID) {
      return (
        <Select
          disabled={!field}
    ...
    // if (FIELD_FOR_NUMBER.includes(field)) {
    //   return (
    //     <InputNumber
    //       placeholder="填写内容值"
    //       min={0}
    ...
    //       style={sizeStyle}
    //       precision={0}
    //       value={parseInt(portValue || '0')}
    //       onChange={(e) => {
    //         console.log(e);
    //         setPortValue(parseInt((e || 0).toFixed(0)) + '');
    //       }}
    //     />
    //   );
    // }

    return <Input disabled={!field} style={sizeStyle} placeholder="填写内容值" />;
  };
  ...
  return (
    <>
      <Form.Item
        name="filter-tuple"
        extra={describeMode && filterTupleExtra}
                        ...
                        <div className={styles.filterForm}>
                          <Input.Group compact style={smallSize ? { flex: '0 0 70%' } : {}}>
                            <Form.Item name={`field_${groupIndex}`}>
                              <Select
                                style={smallSize ? { width: 120 } : { width: 200 }}
                                placeholder="选择字段"
                                onChange={(fieldId) =>
                            ...
                            >
                              <Select
                                ...
                                style={smallSize ? { width: 70 } : { width: 100 }}
                                placeholder="操作方法"
                              ...
                              >
                                ...
                                {/* {supportInOperatorFields.indexOf(selectedField) > -1 ? (
                                  FILTER_RULE_OPERATOR_LIST.map(({ label, value: _value }) => (
                                    <Select.Option key={value} value={_value}>
                                      {label}
                                    </Select.Option>
                                  ))
