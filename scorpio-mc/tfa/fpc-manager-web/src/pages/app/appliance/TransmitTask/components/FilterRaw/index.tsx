                      ...
                      <FormItem name={`group_${groupId}_type`} initialValue={EFilterRawType.ASCII}>
                        <Select
                          ...
                          onChange={(value) => {
                            ...
                            if (elem) {
                              if (
                                value === EFilterRawType.CHINESE ||
                                value === EFilterRawType.ASCII ||
                                value === EFilterRawType.HEX
                              ) {
                                // @ts-ignore
                                elem.innerHTML = `<ul>
                                    <li>中文支持按照UTF-8，GB2312两种编码进行匹配</li>
                                    <li>中文最大长度21，ASCII 64个，16进制128个</li>
                                  </ul>
                                `;
                              } else {
                                // @ts-ignore
                        ...
                        rules={[
                          {
                            validator: async (rule, value) => {
                              ...
                              if (type === EFilterRawType.REGULAR) {
                                ...
                                const utfIndex = value.indexOf('(*UTF8)');
                                const ucpIndex = value.indexOf('(*UCP)');
                                if (utfIndex === 0) {
                                  if (!isReg(value.substr(7))) {
                                    throw new Error('正则表达式格式错误');
                                  }
                                } else if (ucpIndex === 0) {
                                  if (!isReg(value.substr(6))) {
                                    throw new Error('正则表达式格式错误');
                                  }
                                } else {
                                  if (!isReg(value)) {
