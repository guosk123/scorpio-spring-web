...
export async function uploadUpgrade({ formData }: any) {
  return ajax(`${API_VERSION_PRODUCT_V1}/system/upgrade`, {
    type: 'POST',
...
export async function queryUpgradeLogs({ cursor }: { cursor: number }) {
  return ajax(`${API_VERSION_PRODUCT_V1}/system/upgrade/logs?cursor=${cursor}`);
}
...
export async function queryUpgradeInfos() {
  return ajax(`${API_VERSION_PRODUCT_V1}/system/upgrade/infos`);
}
