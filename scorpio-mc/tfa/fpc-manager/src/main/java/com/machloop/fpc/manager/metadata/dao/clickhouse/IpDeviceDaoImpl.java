...
import com.machloop.fpc.manager.metadata.dao.IpDeviceDao;
...
public class IpDeviceDaoImpl implements IpDeviceDao {
  ...
  private static final String TABLE_NAME = "t_fpc_protocol_rtp_log_record";
  ...
  private Map<String, Object> convertMap(Map<String, Object> map, LogRecordQueryVO queryVO) {
    ...
    if (map.containsKey("device_ip")) {
      ...
      if (StringUtils.isNotBlank(deviceIp)
          && StringUtils.startsWith(deviceIp, ManagerConstants.IPV4_TO_IPV6_PREFIX)) {
        deviceIp = StringUtils.substringAfter(deviceIp, ManagerConstants.IPV4_TO_IPV6_PREFIX);
      }
