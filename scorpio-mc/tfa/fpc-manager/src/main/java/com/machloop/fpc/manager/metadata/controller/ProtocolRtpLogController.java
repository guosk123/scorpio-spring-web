...
import com.machloop.fpc.manager.metadata.service.ProtocolRtpLogService;
import com.machloop.fpc.manager.metadata.vo.LogRecordQueryVO;
import com.machloop.fpc.manager.metadata.vo.ProtocolRtpLogVO;
...
public class ProtocolRtpLogController {


  private static final Logger LOGGER = LoggerFactory.getLogger(ProtocolRtpLogController.class);


  @Autowired
  @Qualifier("protocolRtpLogService")
  private LogRecordService<ProtocolRtpLogVO> logRecordService;

  @Autowired
  private ScenarioTaskResultService scenarioTaskResultService;

  @Autowired
  private ProtocolRtpLogService protocolRtpLogService;


  @GetMapping("/protocol-rtp-logs")
  @Secured({"PERM_USER"})
  public Page<ProtocolRtpLogVO> queryLogRecords(
    ...
    Sort sort = new Sort(new Sort.Order(Sort.Direction.fromString(sortDirection), sortProperty),
        new Sort.Order("start_time"), new Sort.Order("flow_id"));
    PageRequest page = new PageRequest(pageNumber, pageSize, sort);

    return logRecordService.queryLogRecords(queryVO, page);
  }

  @GetMapping("/network-segmentation")
  @Secured({"PERM_USER"})
  public Map<String, Map<String, Object>> queryRtpNetworkSegmentation(
      @RequestParam(required = false, defaultValue = "") String startTime,
    ...
    return protocolRtpLogService.queryRtpNetworkSegmentation(queryVO);
  }

  @GetMapping("/protocol-rtp-logs/as-statistics")
  @Secured({"PERM_USER"})
  ...
  @GetMapping("/protocol-rtp-logs/packets")
  @Secured({"PERM_USER"})
  ...
  @GetMapping("/protocol-rtp-logs/{id}")
  @Secured({"PERM_USER"})
  public ProtocolRtpLogVO queryLogRecord(@PathVariable String id, LogRecordQueryVO queryVO) {

    return logRecordService.queryLogRecord(queryVO, id);
  }

  @GetMapping("/protocol-rtp-logs/as-export")
  @Secured({"PERM_USER"})
  public void exportLogRecords(@RequestParam(required = false, defaultValue = "") String startTime,
    ...
    String fileName = StringUtils.equals(fileType, Constants.EXPORT_FILE_TYPE_CSV) ? "rtp_logs.csv"
        : "rtp_logs.xlsx";
    ...
    try (OutputStream out = response.getOutputStream();) {
      ...
      LOGGER.warn("export rtp logs error ", e);
    }
  }
