...
import com.machloop.fpc.manager.handler.QueryTaskRejectedExecutionHandler;
...
public class FlowLogDataServiceImpl implements FlowLogDataService {
  ...
  private static final ThreadPoolExecutor executor = new ThreadPoolExecutor(0, 100, 3,
  ...
  /**
   * @see com.machloop.fpc.manager.metadata.service.FlowLogDataService#countFlowLogDataGroupByProtocol(com.machloop.fpc.manager.metadata.vo.LogCountQueryVO)
   */
  @Override
  public Map<String, Long> countFlowLogDataGroupByProtocol(LogCountQueryVO queryVO) {
    ...
    if (StringUtils.isNotBlank(queryVO.getSrcIp())
        || StringUtils.equals(queryVO.getSourceType(), FpcConstants.SOURCE_TYPE_PACKET_FILE)) {
