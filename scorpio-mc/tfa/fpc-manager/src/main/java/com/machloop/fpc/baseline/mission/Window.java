...
public class Window {
  ...
  public Window(int size, int precision, int bucketSeconds, int bucketGapSconds, boolean offset) {
    this.precision = precision;
    this.period = precision;
    this.bucketSeconds = bucketSeconds;
    this.bucketGapSeconds = bucketGapSconds;
  ...
  public List<Tuple2<Date, Date>> getRanges() {
    ...
    for (long bucket : deltaBuckets) {
      if (offset) {
        ...
        long start = precisionCurrent - bucket;
        long end = start + bucketSeconds * 1000;
        ranges.add(Tuples.of(new Date(start), new Date(end)));
      }
    }
    return ranges;
  ...
  /**
   * 小时环比（告警，由于告警条件与基线窗口不一致，导致小时环比时，基线应该从前一小时开始计算。例：回顾周期为1,10:09分判断是够需要告警时要对比09:00-10:00时间范围内计算得到的基线值才准确）
   * @param size 窗口大小
   * @return
   */
  public static Window newHourOnHourWindowFromAlert(int size) {
    return new Window(size, Constants.ONE_HOUR_SECONDS, Constants.ONE_HOUR_SECONDS,
        Constants.ONE_HOUR_SECONDS, false);
  }
  ...
  public static Window createWindow(int size, WindowModelEnum windowModel) {
    ...
    switch (windowModel) {
      ...
      case LAST_N_HOURS:
        ...
        break;
      case LAST_N_HOURS_ALERT:
        window = newHourOnHourWindowFromAlert(size);
        break;
      default:
