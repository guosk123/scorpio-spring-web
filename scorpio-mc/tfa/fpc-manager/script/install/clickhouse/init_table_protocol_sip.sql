CREATE TABLE IF NOT EXISTS fpc.t_fpc_protocol_sip_log_record
(
    ...
    INDEX i_seq_num seq_num TYPE minmax GRANULARITY 4,
    INDEX i_call_id call_id TYPE bloom_filter GRANULARITY 4,
    INDEX i_request_uri request_uri TYPE bloom_filter GRANULARITY 4,
    INDEX i_status_code status_code TYPE bloom_filter GRANULARITY 4
)
ENGINE = MergeTree
...
/* Meterialized Viewes */
CREATE MATERIALIZED VIEW IF NOT EXISTS v_fpc_protocol_dict_sip_sdp
TO t_fpc_protocol_dict
AS SELECT
    'sip' as protocol,
    'sdp' as field,
    arrayJoin(mapKeys(sdp)) as key
